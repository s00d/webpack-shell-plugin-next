...
var WebpackShellPlugin = /** @class */ (function () {
    ...
    WebpackShellPlugin.prototype.handleScript = function (script) {
        ...
        var result = child_process_1.spawnSync(command, args, { stdio: this.logging ? ['inherit', 'inherit', 'pipe'] : undefined, env: env, shell: true });
    ...
    WebpackShellPlugin.prototype.handleScriptAsync = function (script) {
        ...
        var env = Object.create(global.process.env);
        env = Object.assign(env, this.env);
        var proc = child_process_1.spawn(command, args, { stdio: 'inherit', env: env, shell: true });
        if (this.logging) {
            proc.on('error', function (err) {
                _this.error("stderr error " + command + " " + args.join(' ') + ": " + err.message);
            });
        }
